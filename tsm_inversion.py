...
if __name__ == '__main__':
    ...
    train_data = train_data[(train_data['TSM(mg/L)'] > 4)]
    test_data = test_data[(test_data['TSM(mg/L)'] > 4)]
    col = ['HJ2A_1', 'HJ2A_2', 'HJ2A_3', 'HJ2A_4', 'HJ2A_5', 'TSM(mg/L)']
    ...
    with open('./Data/tsm_random_forest_model.pkl', 'wb') as f:
    ...
    plt.plot([0, 175], [0, 175], 'k', linewidth=1.5, label='1:1')
    ...
    plt.text(max_value * 0.75, max_value * 0.04, text3,
             fontdict=fontdict)
    plt.xlabel('Measured TSM(mg/L)', fontdict=fontdict)
    plt.ylabel('Derived TSM(mg/L)', fontdict=fontdict)
    ...
    plt.savefig('./picture/tsm_train_data.png', dpi=300)
    plt.show()
    ...
    plt.legend(loc='best', prop=fontdict, frameon=False)
    plt.axis([0, 175, 0, 175])
    plt.savefig('./picture/tsm_test_data.png', dpi=300)
    plt.show()
