...
if __name__ == '__main__':
    ...
    train_data = train_data[(train_data['Chl-a(μg/L)'] < 100)]
    test_data = test_data[(test_data['Chl-a(μg/L)'] < 100)]
    col = ['HJ2A_1', 'HJ2A_2', 'HJ2A_3', 'HJ2A_4', 'HJ2A_5', 'Chl-a(μg/L)']
    ...
    with open('./Data/chla_random_forest_model.pkl', 'wb') as f:
    ...
    x_ = np.arange(min_value, 100, 0.01)
    ...
    plt.plot([0, 100], [0, 100], 'k', linewidth=1.5, label='1:1')
    ...
    plt.text(max_value * 0.75, max_value * 0.04, text3,
             fontdict=fontdict)
    plt.xlabel('Measured Chl-a(μg/L)', fontdict=fontdict)
    plt.ylabel('Derived Chl-a(μg/L)', fontdict=fontdict)
    ...
    plt.savefig('./picture/chla_train_data.png', dpi=300)
    plt.show()
    ...
    # 设置图例
    fontdict = {'family': 'Times New Roman', 'size': 12, 'style': 'normal'}
    plt.text(100 * 0.7, max_value * 0.04, text3,
             fontdict=fontdict)
    plt.xlabel('Measured Chl-a(μg/L)', fontdict=fontdict)
    plt.ylabel('Derived Chl-a(μg/L)', fontdict=fontdict)
    plt.xticks(fontname='Times New Roman', fontsize=10)
    ...
    plt.legend(loc='best', prop=fontdict, frameon=False)
    plt.axis([0, 100, 0, 100])
    plt.savefig('./picture/chla_test_data.png', dpi=300)
    plt.show()
